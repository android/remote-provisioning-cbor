...
public class CborUtil {
  ...
  public static CBORObject encodeEncryptMessage(
    ...
    unprotectedHeadersRecip.Add(
        ...
        CryptoUtil.cborEncodeX25519PubKey(
            (X25519PublicKeyParameters) ephemeralKeyPair.getPublic()));
  ...
  private static CBORObject getRecipient(CBORObject recipients) throws CborException {
    if (recipients.getType() != CBORType.Array) {
      throw new CborException(
          "Recipient list of ProtectedData has the wrong type",
          CBORType.Array,
          recipients.getType(),
    ...
    if (recipient.getType() != CBORType.Array) {
      throw new CborException(
          "Recipient of ProtectedData has the wrong type",
          CBORType.Array,
          recipient.getType(),
          CborException.TYPE_MISMATCH);
    }
    if (recipient.size() != COSE_RECIPIENT_LENGTH) {
      throw new CborException(
          "Recipient of ProtectedData has the wrong length",
  ...
  public static byte[] extractEekId(byte[] cborProtectedData) throws CborException {
    ...
    if (encMsg.getType() != CBORType.Array) {
      throw new CborException(
          "ProtectedData has the wrong type",
          CBORType.Array,
          encMsg.getType(),
          CborException.TYPE_MISMATCH);
    }
    if (encMsg.size() != COSE_ENCRYPT_LENGTH) {
      throw new CborException(
          "ProtectedData has the wrong length",
          COSE_ENCRYPT_LENGTH,
          encMsg.size(),
          CborException.INCORRECT_LENGTH);
    }
  ...
  public static CBORObject decodeEncryptMessage(
    ...
    CBORObject ephemeralPublicKeyCbor =
        CBORObject.DecodeFromBytes(
            unprotectedHeadersRecip.get(HeaderKeys.ECDH_EPK.AsCBOR()).GetByteString());
