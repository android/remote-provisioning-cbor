  ...
  private void parseDeviceInfo(CBORObject deviceInfo) throws CborException {
    if (deviceInfo.getType() != CBORType.Map) {
      throw new CborException(
          "DeviceInfo type wrong", CBORType.Map, deviceInfo.getType(), CborException.TYPE_MISMATCH);
    }
    mBrand = Optional.ofNullable(deviceInfo.get("brand")).map(x -> x.AsString()).orElse("");
    mManufacturer =
        Optional.ofNullable(deviceInfo.get("manufacturer")).map(x -> x.AsString()).orElse("");
    mProduct = Optional.ofNullable(deviceInfo.get("product")).map(x -> x.AsString()).orElse("");
    mModel = Optional.ofNullable(deviceInfo.get("model")).map(x -> x.AsString()).orElse("");
    mBoard = Optional.ofNullable(deviceInfo.get("board")).map(x -> x.AsString()).orElse("");
    mVbState = Optional.ofNullable(deviceInfo.get("vb_state")).map(x -> x.AsString()).orElse("");
    mBootloaderState =
        Optional.ofNullable(deviceInfo.get("bootloader_state")).map(x -> x.AsString()).orElse("");
    mVbmetaDigest =
        Optional.ofNullable(deviceInfo.get("vbmeta_digest")).map(x -> x.AsString()).orElse("");
    mOsVersion =
        Optional.ofNullable(deviceInfo.get("os_version")).map(x -> x.AsString()).orElse("");
    mSystemPatchLevel =
        Optional.ofNullable(deviceInfo.get("system_patch_level")).map(x -> x.AsInt32()).orElse(-1);
    mBootPatchLevel =
        Optional.ofNullable(deviceInfo.get("boot_patch_level")).map(x -> x.AsInt32()).orElse(-1);
    mVendorPatchLevel =
        Optional.ofNullable(deviceInfo.get("vendor_patch_level")).map(x -> x.AsInt32()).orElse(-1);
  }
